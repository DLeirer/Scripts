...
path_to_scripts <- "~/Documents/A_Year_1_PhD/GAP_Data/2014_12_08_GAP_preprocessing_pipeline_modified"
...
# project directory
project_dir <- "~/Documents/A_Year_1_PhD/GAP_Data/2014_12_08_GAP_preprocessing_pipeline_modified"
...
out_dir <- paste(project_dir,"/","GAP_DL_FINAL_lumi_processing_09_12_2014_1418134305",sep="")
...
gs_report  <- "~/Documents/A_Year_1_PhD/GAP_Data/2014_12_08_GAP_preprocessing_pipeline_modified/final_reports_genomestudio/Sample_and_Control_Probe_Profile_FinalReport.txt"
gs_probe   <- "~/Documents/A_Year_1_PhD/GAP_Data/2014_12_08_GAP_preprocessing_pipeline_modified/final_reports_genomestudio/Group_Probe_Profile_Final_Report.txt" 
gs_sample  <- "~/Documents/A_Year_1_PhD/GAP_Data/2014_12_08_GAP_preprocessing_pipeline_modified/final_reports_genomestudio/sample_table_Final_Report.txt" 
gs_control <- "~/Documents/A_Year_1_PhD/GAP_Data/2014_12_08_GAP_preprocessing_pipeline_modified/final_reports_genomestudio/control_probe_profile_Final_Report.txt" 
anno_table <- "~/Documents/A_Year_1_PhD/GAP_Data/2014_12_08_GAP_preprocessing_pipeline_modified/final_reports_genomestudio/probe_annotation_Final_Report.txt"

# sample information
# FILE NAME must contain : Sample.ID,SEX,GROUPS,TISSUE,PHENOTYPE,Study_ID
pheno_file <- "~/Documents/A_Year_1_PhD/GAP_Data/2014_12_08_GAP_preprocessing_pipeline_modified/final_reports_genomestudio/pheno_info.txt" 

# batch information
tech_pheno_file <-"~/Documents/A_Year_1_PhD/GAP_Data/2014_12_08_GAP_preprocessing_pipeline_modified/final_reports_genomestudio/batch_info.txt"
...
## get SRY gene postion
sry_raw <- fData(eset_raw)$ILMN_GENE=="PRKY";
sry_bgcor <- fData(eset_bg)$ILMN_GENE=="PRKY";
table(sry_raw)
table(sry_bgcor)


## get XIST gene expression signal
sry_gx_raw  <- exprs(eset_raw[sry_raw, ]  )
sry_gx_raw <- as.data.frame( t(sry_gx_raw));
sry_gx_bgcor <- exprs(eset_bg[sry_bgcor , ]  )
sry_gx_bgcor <- as.data.frame( t(sry_gx_bgcor));

## cobine raw and bkCro gx data
sry_gx <- cbind(sry_gx_raw,sry_gx_bgcor)
colnames(sry_gx) <- c("raw_SRY","bgcor_SRY")
str(sry_gx)
sry_gx$neg_2sd <- neg_2sd
sry_gx$neg_max <- neg_max
sry_gx$neg_mean <- neg_mean
sry_gx$neg_sd <- neg_sd
...
sry_gx$sry_z <-  ( sry_gx$bgcor_SRY - sry_gx$neg_mean ) / sry_gx$neg_sd
sry_gx$Sample.ID <- rownames(sry_gx)

# illumina detection p value for xist
sry_gx$sry_illumina_detection_p <- as.numeric(detection(eset_bg[fData(eset_bg)$ILMN_GENE=="PRKY",]))
sry_gx$sry_illumina_detection_p <- ifelse(sry_gx$sry_illumina_detection_p  == 0.00000, 0.00001,sry_gx$sry_illumina_detection_p )
...
sry_gx$sry_gender <- ifelse(sry_gx$bgcor_SRY<=(sry_gx$neg_mean*59/170), "FEMALE","MALE") ##this was just arbitarily adjusted based on visuals. 
str(sry_gx)


mean(sry_gx$bgcor_SRY)
mean(sry_gx$neg_mean)
...
sry_gx$clinical_gender <- pData(eset_bg)$SEX

# gender based on illumina detecetion p value
sry_gx$sry_illumina_detection_p_gender <- ifelse(sry_gx$sry_illumina_detection_p <= 0.01,"MALE","FEMALE") ## 0.05 makes them all FEMALE!

# flag gender FAIL
sry_gx$gender_FAIL <- ifelse(sry_gx$sry_gender == sry_gx$clinical_gender, "PASS","GENDER_FAIL" )
sry_gx$gender_FAIL_illumina_detection_p <- ifelse(sry_gx$sry_illumina_detection_p_gender == sry_gx$clinical_gender, "PASS","GENDER_FAIL" )
# head
head(sry_gx)

# gender_concordance
gender_concordance <- round(sum(sry_gx$sry_gender==sry_gx$clinical_gender)/dim(sry_gx)[1],3)
cat(" Gender Concordance=[",gender_concordance,"]","\r","\n")

# table SEX
table(sry_gx$clinical_gender);
table(sry_gx$sry_gender);
table(sry_gx$sry_illumina_detection_p_gender);


# tables SEX compare
table(sry_gx$sry_gender, sry_gx$clinical_gender);
table(sry_gx$sry_illumina_detection_p_gender, sry_gx$clinical_gender);

#percent_gender_match
percent_gender_match <- round(sum(sry_gx$sry_gender==sry_gx$clinical_gender)/dim(sry_gx)[1],3)
cat(" Percent Gender Match=[",percent_gender_match,"]","\r","\n")

# Density plots with semi-transparent fill of SEX CALLS
df <- sry_gx[,c("bgcor_SRY", "clinical_gender", "sry_gender", "sry_illumina_detection_p_gender")]
head(df)

# Find the mean of each group
# library(plyr)
cdf_clinical_gender <- ddply(df, "clinical_gender", summarise, mean=mean(log2(bgcor_SRY)))
cdf_sry_gender <- ddply(df, "sry_gender", summarise, mean=mean(log2(bgcor_SRY)))
...
# ggplots
ggplot(df, aes(x=log2(bgcor_SRY), fill=clinical_gender )) + geom_histogram(binwidth=.5, alpha=.5, position="identity")
ggplot(df, aes(x=log2(bgcor_SRY), fill=sry_gender )) + geom_histogram(binwidth=.5, alpha=.5, position="identity")
ggplot(df, aes(x=log2(bgcor_SRY), fill=sry_illumina_detection_p_gender )) + geom_histogram(binwidth=.5, alpha=.5, position="identity")


ggplot(df, aes( y=log2(bgcor_SRY), x=clinical_gender, fill=clinical_gender)) + geom_boxplot() +  guides(fill=FALSE) + coord_flip()
ggplot(df, aes( y=log2(bgcor_SRY), x=sry_gender, fill=sry_gender)) + geom_boxplot() +  guides(fill=FALSE) + coord_flip()
...
robust_m0  <- rlm( PC1 ~ multivariate_model_terms, data=tech_batch)
robust_m0  <- rlm( PC1 ~ . , data=tech_batch)
...
robust_m1 <- update(robust_m0, -c(2,5,6))
summary(robust_m1)


robust_m2 <- update(robust_m1, -c(4))
summary(robust_m1)
...
for(probe in new_probe_names){
    ...
    lm_probe <-  rlm( lm_model , data=eset_bg_log2_rsn_regression_input)
